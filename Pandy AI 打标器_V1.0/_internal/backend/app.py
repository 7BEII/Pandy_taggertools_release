...
def export_images():
    ...
    try:
        ...
        if output_type == 'zip':
            # 导出为ZIP
            if not output_path:
                output_path = os.path.join(TRAINING_DATAS_DIR, f"dataset_{timestamp}.zip")
            elif not output_path.endswith('.zip'):
                output_path = output_path + '.zip'
            
            # 确保目录存在
            os.makedirs(os.path.dirname(output_path) if os.path.dirname(output_path) else TRAINING_DATAS_DIR, exist_ok=True)
            ...
            # 导出为文件夹
            if not output_path:
                output_path = os.path.join(TRAINING_DATAS_DIR, f"dataset_{timestamp}")
