...
class APIHandler:
    ...
    def translate_text(text, api_key, base_url, model, target_lang=None):
        ...
        # 根据目标语言设置提示词
        if target_lang == 'en':
            system_prompt = "You are a professional translator. Translate the given Chinese text to English. Only output the translated English text, nothing else."
        elif target_lang == 'zh':
            system_prompt = "You are a professional translator. Translate the given English text to Chinese. Only output the translated Chinese text, nothing else."
        else:
            # 自动检测语言并翻译
        ...
        print(f"[翻译请求] 开始翻译 | 文本长度: {len(text)} | 模型: {text_model} | 目标语言: {target_lang or '自动'}")
