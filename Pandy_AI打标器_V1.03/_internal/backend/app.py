        ...
        def process_batch():
            ...
            for idx, pair_id in enumerate(ids, start=1):
                ...
                if not image_path:
                    ...
                    processing_tasks[task_id]['failed_ids'].append(pair_id)
                    continue
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if '429' in error_str and attempt < max_retries:
                            print(f"[重试] {pair_id} 遇到429限流，等待2秒后重试...")
                            import time
                            time.sleep(2)
                        elif attempt < max_retries:
                            print(f"[重试] {pair_id} 处理失败，尝试重试 ({attempt + 1}/{max_retries})...")
                            import time
                            time.sleep(2)
                        else:
                            break
                ...
                if not success:
                    print(f"Failed to process pair {pair_id}: {last_error}")
                    pairs_data[pair_id]['status'] = 'error'
                    pairs_data[pair_id]['error_message'] = str(last_error)
                    processing_tasks[task_id]['failed'] += 1
                    processing_tasks[task_id]['failed_ids'].append(pair_id)
...
def translate_text():
    ...
    try:
        ...
        model_id = data.get('model_id', None)  # 可选的翻译模型ID
        provider_key = data.get('provider', None)  # 可选的API厂商
        
        # 调试日志：打印接收到的参数
        print(f"[翻译API] 接收参数: model_id={model_id}, provider={provider_key}, target_lang={target_lang}")
        ...
        # 如果指定了厂商，使用指定的厂商；否则使用当前厂商
        if provider_key and provider_key in apikey_config['providers']:
        ...
        # 如果指定了翻译模型，使用指定的模型；否则使用默认翻译模型
        if model_id:
            model = model_id
        else:
            model = 'Qwen/Qwen2.5-7B-Instruct'  # 默认翻译模型
        
        print(f"[翻译API] 最终使用: model={model}, provider={provider}")
        
        if not api_key:
            return jsonify({"success": False, "message": f"请先配置 {provider} 的 API Key"}), 400
...
def tag_single_image(img_id):
    ...
    try:
        ...
        # 获取裁剪参数（如果有）
        crop_params = images_data[img_id].get('crop_params')
        
        result = APIHandler.call_vision_api(
            image_path, system_prompt, user_prompt, api_key, base_url, model, crop_params
        )
    ...
    try:
        ...
        def process_batch():
            ...
            for idx, img_id in enumerate(ids, start=1):
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        result = APIHandler.call_vision_api(
                            ...
                            system_prompt, user_prompt, api_key, base_url, model, crop_params
                        )
                        images_data[img_id]['text'] = result
                        images_data[img_id]['status'] = 'success'
                        processing_tasks[task_id]['completed'] += 1
                        ...
                        # 如果是429错误（速率限制），等待后重试
                        if '429' in error_str and attempt < max_retries:
                            print(f"[重试] {img_id} 遇到429限流，等待2秒后重试...")
                            import time
                            time.sleep(2)
                        elif attempt < max_retries:
                            print(f"[重试] {img_id} 处理失败，尝试重试 ({attempt + 1}/{max_retries})...")
                            import time
                            time.sleep(2)
                        else:
                            break
                ...
                if not success:
                    print(f"Failed to process {img_id}: {last_error}")
                    images_data[img_id]['status'] = 'error'
                    images_data[img_id]['error_message'] = str(last_error)
                    processing_tasks[task_id]['failed'] += 1
                    processing_tasks[task_id]['failed_ids'].append(img_id)
    ...
    try:
        ...
        for img_id in ids:
            if img_id in images_data:
                updated_images.append(images_data[img_id])
...
@app.route('/api/batch/crop', methods=['POST'])
def batch_crop():
    """批量自定义裁剪图片（保留原图，只保存裁剪参数）"""
    try:
        data = request.get_json(silent=True) or {}
        crop_data = data.get('crop_data', [])  # [{id, crop_x, crop_y, crop_width, crop_height}]
        target_width = int(data.get('target_width', 1024))
        target_height = int(data.get('target_height', 1024))
        
        print(f"[batch_crop] Received crop_data: {len(crop_data)} items, target: {target_width}x{target_height}")
        
        if not crop_data:
            return jsonify({"success": False, "message": "请提供裁剪数据"}), 400
        ...
        for item in crop_data:
            img_id = item.get('id')
            print(f"[batch_crop] Processing img_id: {img_id}, exists: {img_id in images_data}")
            
            if img_id not in images_data:
                print(f"[batch_crop] Image not found: {img_id}")
                failed_count += 1
                continue
            ...
            if not img_path or not os.path.exists(img_path):
                print(f"[batch_crop] Path not found: {img_path}")
                failed_count += 1
                continue
            
            # 获取裁剪参数
            crop_x = float(item.get('crop_x', 0))
            crop_y = float(item.get('crop_y', 0))
            crop_width = float(item.get('crop_width', 1))
            crop_height = float(item.get('crop_height', 1))
            
            print(f"[batch_crop] Cropping: x={crop_x}, y={crop_y}, w={crop_width}, h={crop_height}")
            
            # 生成裁剪后的缩略图（不保存原图）
            thumb_base64, _ = ImageProcessor.crop_image_to_base64(
                img_path, crop_x, crop_y, crop_width, crop_height,
            ...
            if thumb_base64:
                ...
                # 更新显示用的缩略图（裁剪后的）
                img['cropped_thumbnail'] = thumb_base64
                # 保留原始缩略图用于重新裁剪
                if 'original_thumbnail' not in img:
                    img['original_thumbnail'] = img.get('thumbnail')
                # 显示裁剪后的缩略图
                img['thumbnail'] = thumb_base64
                # 更新显示尺寸（裁剪后的尺寸）
                img['display_width'] = target_width
                img['display_height'] = target_height
                ...
                print(f"[batch_crop] Success: {img_id}")
            else:
                failed_count += 1
                results.append({
                    'id': img_id,
                ...
                print(f"[batch_crop] Failed: {img_id}")
        
        message = f"已裁剪 {cropped_count} 张图片"
        if failed_count > 0:
            message += f"，{failed_count} 张失败"
        
        print(f"[batch_crop] Done: {message}")
...
@app.route('/api/batch/reset-crop', methods=['POST'])
def batch_reset_crop():
    """重置裁剪，恢复原图显示"""
    try:
        data = request.get_json(silent=True) or {}
        ids = data.get('ids', [])
        ...
        if not ids:
            return jsonify({"success": False, "message": "请选择要重置的图片"}), 400
        ...
        for img_id in ids:
            ...
            # 清除裁剪参数
            if 'crop_params' in img:
                del img['crop_params']
            
            # 恢复原始缩略图
            if 'original_thumbnail' in img:
                img['thumbnail'] = img['original_thumbnail']
                del img['original_thumbnail']
            
            # 清除裁剪后的缩略图
            if 'cropped_thumbnail' in img:
                del img['cropped_thumbnail']
            
            # 清除显示尺寸
            if 'display_width' in img:
                del img['display_width']
            if 'display_height' in img:
                del img['display_height']
            
            reset_count += 1
        
        return jsonify({
            "success": True,
            "message": f"已重置 {reset_count} 张图片的裁剪",
...
@app.route('/api/batch/crop-preview', methods=['POST'])
def batch_crop_preview():
    """获取批量裁剪预览"""
    try:
        data = request.get_json(silent=True) or {}
        img_id = data.get('id')
        ...
        crop_width = data.get('crop_width', 1)
        crop_height = data.get('crop_height', 1)
        ...
        preview = ImageProcessor.get_crop_preview(
            img_path, crop_x, crop_y, crop_width, crop_height,
            target_width, target_height
        )
        ...
        if preview:
            ...
            return jsonify({"success": False, "message": "生成预览失败"}), 500
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500


@app.route('/api/batch/crop-individual', methods=['POST'])
def batch_crop_individual():
    """批量自定义裁剪图片（每张图片独立尺寸，保留原图）"""
    try:
        data = request.get_json(silent=True) or {}
        crop_data = data.get('crop_data', [])  # [{id, crop_x, crop_y, crop_width, crop_height, target_width, target_height}]
        fill_background = data.get('fill_background', False)  # 是否填充透明背景
        background_color = data.get('background_color', '#FFFFFF')  # 背景填充颜色
        
        print(f"[batch_crop_individual] Received crop_data: {len(crop_data)} items, fill_bg={fill_background}, bg_color={background_color}")
        ...
        for item in crop_data:
            img_id = item.get('id')
            target_width = int(item.get('target_width', 1024))
            target_height = int(item.get('target_height', 1024))
            
            print(f"[batch_crop_individual] Processing img_id: {img_id}, target: {target_width}x{target_height}")
            
            if img_id not in images_data:
                print(f"[batch_crop_individual] Image not found: {img_id}")
                failed_count += 1
                continue
            ...
            if not img_path or not os.path.exists(img_path):
                print(f"[batch_crop_individual] Path not found: {img_path}")
                failed_count += 1
                continue
            ...
            crop_width = float(item.get('crop_width', 1))
            crop_height = float(item.get('crop_height', 1))
            image_scale = float(item.get('image_scale', 1.0))  # 图片缩放比例
            offset_x = float(item.get('offset_x', 0))  # 图片X偏移
            offset_y = float(item.get('offset_y', 0))  # 图片Y偏移
            
            print(f"[batch_crop_individual] Cropping: x={crop_x}, y={crop_y}, w={crop_width}, h={crop_height}, scale={image_scale}, offset=({offset_x}, {offset_y})")
            ...
            if thumb_base64:
                ...
                img['crop_params'] = {
                    ...
                    'target_width': target_width,
                    'target_height': target_height,
                    'image_scale': image_scale,  # 保存缩放比例
                    'offset_x': offset_x,  # 保存X偏移
                    'offset_y': offset_y,   # 保存Y偏移
                    'fill_background': fill_background,  # 保存是否填充背景
                    'background_color': background_color  # 保存背景颜色
                }
                # 更新显示用的缩略图（裁剪后的）
                img['cropped_thumbnail'] = thumb_base64
                # 保留原始缩略图用于重新裁剪
                ...
                print(f"[batch_crop_individual] Success: {img_id} -> {target_width}x{target_height}")
            else:
                failed_count += 1
                results.append({
                    'id': img_id,
                ...
                print(f"[batch_crop_individual] Failed: {img_id}")
        
        message = f"已裁剪 {cropped_count} 张图片"
        if failed_count > 0:
            message += f"，{failed_count} 张失败"
        
        print(f"[batch_crop_individual] Done: {message}")
...
def batch_resize():
    ...
    try:
        ...
        if not ids:
            return jsonify({"success": False, "message": "请选择要处理的图片"}), 400
        ...
        for img_id in ids:
            if img_id in images_data:
                ...
                if img_path and os.path.exists(img_path):
                    # 执行缩放（允许放大）
                    if ImageProcessor.resize_image_by_longest_edge(img_path, max_size, allow_upscale=True):
                        resized_count += 1
                        # 更新缩略图
                        thumb = ImageProcessor.create_thumbnail(img_path)
                        if thumb:
                            img['thumbnail'] = thumb
                        # 更新尺寸信息
                        try:
        ...
        message = f"已处理 {resized_count} 张图片"
        if skipped_count > 0:
            message += f"，{skipped_count} 张图片尺寸已为 {max_size}px 无需处理"
        ...
        for img_id in ids:
            if img_id in images_data:
                ...
                export_data.append({
                    ...
                    'text': img.get('text', ''),
                    'name': img.get('name', ''),
                    'export_name': img.get('export_name', ''),  # 添加export_name
                    'crop_params': img.get('crop_params')  # 添加裁剪参数
                })
        ...
        if output_type == 'zip':
            ...
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                for idx, item in enumerate(export_data):
                    src_path = item['path']
                    # 优先使用export_name，否则使用原始name
                    base_name = item.get('export_name') or os.path.splitext(item['name'])[0]
                    ...
                    # 检查是否有裁剪参数
                    crop_params = item.get('crop_params')
                    if crop_params:
                        # 有裁剪参数，使用裁剪后的图片
                        _, cropped_bytes = ImageProcessor.crop_image_to_base64(
                            src_path,
                            crop_params['crop_x'],
                            crop_params['crop_y'],
                            crop_params['crop_width'],
                            crop_params['crop_height'],
                            crop_params['target_width'],
                            crop_params['target_height'],
                            fill_background=crop_params.get('fill_background', False),
                            background_color=crop_params.get('background_color', '#FFFFFF'),
                            image_scale=crop_params.get('image_scale', 1.0),
                            offset_x=crop_params.get('offset_x', 0.0),
                            offset_y=crop_params.get('offset_y', 0.0)
                        )
                        if cropped_bytes:
                            # 裁剪后的图片是JPEG格式，如果需要PNG则转换
                            if image_format == 'png':
                                ...
                                zf.writestr(f"{base_name}{target_ext}", cropped_bytes)
                        else:
                            # 裁剪失败，使用原图
                            zf.write(src_path, f"{base_name}{target_ext}")
                    elif src_ext == target_ext or (image_format == 'jpg' and src_ext in ['.jpg', '.jpeg']):
                        # 格式相同，直接复制
                        zf.write(src_path, f"{base_name}{target_ext}")
                    else:
                        # 需要转换格式
